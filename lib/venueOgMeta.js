...
export function injectPageSeoIntoHtml(html, meta) {
  ...
  if (ogType) setMetaContent('property', 'og:type', ogType);
  if (imageUrl) {
  ...
  out = out.replace(
    /<script[^>]*(?:data-seo-venue|data-seo-landing|data-seo-listing|data-seo-breadcrumb|data-seo-blog)="1"[^>]*>[\s\S]*?<\/script>/gi,
    ''
  );
  out = out.replace(/<noscript[^>]*data-seo-crawl-links="1"[^>]*>[\s\S]*?<\/noscript>/gi, '');
  out = out.replace(/<article[^>]*(?:data-seo-static|data-seo-crawl-article)="1"[^>]*>[\s\S]*?<\/article>/gi, '');
