...
const MobileView: React.FC<MobileViewProps> = (props) => {
    ...
    return (
        <div className="relative h-[calc(100vh-64px)] overflow-hidden">
            ...
            <div 
                className={`fixed inset-x-0 bottom-0 z-40 transition-all duration-500 transform ${getTranslation()}`}
                ...
                {selectedVenue && (
                    <div className={`rounded-t-[36px] shadow-[0_-12px_40px_rgba(0,0,0,0.15)] h-[90vh] flex flex-col ${darkMode ? 'bg-gray-900 text-white border-t border-gray-800' : 'bg-white text-gray-900 border-t border-gray-100'}`}>
                        <div 
                            ...
                            onClick={() => setSheetState(prev => prev === 'mini' ? 'expanded' : 'mini')}
                        >
                        ...
                        <button
                            ...
                            className={`absolute top-3 right-4 w-11 h-11 rounded-full flex items-center justify-center text-3xl font-light z-50 transition-all ${darkMode ? 'bg-gray-800 text-gray-500' : 'bg-gray-100 text-gray-400'}`}
                        >
                            ×
                        </button>
