 ...
 * Returns inject-ready fields for injectPageSeoIntoHtml (canonicalUrl, imageUrl, jsonLdStrings).
 */
...
export function buildBlogListPageMeta({ posts = [], origin, lang = 'en' } = {}) {
  ...
  return {
    ...
    ogType: 'website',
    jsonLdStrings: [jsonLdString(blogLd), jsonLdString(listLd)],
    jsonLdMarker: 'data-seo-blog="1"',
    noscriptHtml,
    staticBodyHtml: '',
  };
}
...
export function buildBlogPostPageMeta(post, { origin, lang = 'en' } = {}) {
  ...
  return {
    ...
    imageUrl: image,
    ogType: 'article',
    jsonLdStrings: [jsonLdString(jsonLd)],
    jsonLdMarker: 'data-seo-blog="1"',
    staticBodyHtml,
  };
}
