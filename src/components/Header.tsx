        ...
        <header className={`sticky top-0 z-[60] ${darkMode ? 'bg-gray-900/95' : 'bg-white/95'} backdrop-blur-md border-b ${darkMode ? 'border-gray-800' : 'border-gray-200'} shadow-sm`}>
            <div className="container mx-auto px-4 py-3 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 md:gap-4">
                    <a 
                        href="https://www.theground.io" 
                        target="_blank" 
                        rel="noopener noreferrer"
                        className="hidden sm:flex px-4 py-2 bg-[#007a67] text-white rounded-[8px] font-[700] text-[12px] shadow-sm hover:brightness-110 active:scale-95 transition-all"
                    >
                        {t('findEvents')}
                    </a>
                    ...
                    <div className={`flex ${darkMode ? 'bg-gray-800' : 'bg-gray-100' } rounded-[8px] p-1`}>
                    ...
                    {isAdmin && (
                        <button onClick={onLogout} className="hidden sm:block px-4 py-2 bg-red-500 text-white rounded-[8px] font-bold text-xs shadow-lg">
                            {t('logout')}
                        </button>
                    )}
                </div>
